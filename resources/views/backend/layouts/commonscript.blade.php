...
if(url.search('blob:') == -1){
  ...
  return false;
	...
	$('.form-control[type="url"], .form-control.url').on('keyup', function (e) {
    var test = $(this).val().trim();

    // Simple URL regex pattern
    var urlPattern = /^(https?:\/\/)?([\w\-]+\.)+[\w\-]+(\/[\w\-./?%&=]*)?$/i;

    // Validate only if input is not empty
    if (test && urlPattern.test(test)) {
        $(this).nextAll('.errorSpan').remove(); // Remove existing error if valid
    } else {
        $(this).nextAll('.errorSpan').remove(); // Remove old error
        $(this).after('<span class="text-danger errorSpan">Invalid URL</span>');
        e.preventDefault();
    }
});
...
$('input[type="url"]').on('keyup', function(e) {
    ...
    var isValid = urlPattern.test(value); // Boolean: is it a valid URL?

    if (isValid) {
        $(this).nextAll('.errorSpan').remove(); // Remove error if valid
    } else {
        $(this).nextAll('.errorSpan').remove(); // Clear existing errors
        $(this).after('<span class="text-danger errorSpan">Invalid character</span>');
        e.preventDefault();
    }
});
...
function imageheightwidth_mainbanner(id, width, height) {

if ((width == 1808) && (height == 750)) {
	return 'true';
} else {
