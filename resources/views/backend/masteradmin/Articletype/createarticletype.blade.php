...
@section('content')
    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header text-white card-header-main">{{ isset($edit_f) ? 'Update' : 'Add' }} Article type
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        @if (Auth::user()->role_id = 1)
                            @if (isset($edit_f))
                                <form id="formiid" method="POST" action="{{ route('masteradmin.updatearticletype') }}"
                                    enctype="multipart/form-data">
                                @else
                                    <form id="formiid" method="POST" action="{{ route('masteradmin.storearticletype') }}"
                                        enctype="multipart/form-data">
                            @endif
                            @if (isset($edit_f))
                                <form id="formiid" method="POST" action="{{ route('masteradmin.updatearticletype') }}"
                        ...
                        <div class="row mb-3">
                            @if (isset($edit_f))
                                ...
                                @if (isset($keydata->id))
                                    @foreach ($keydata->article_sub as $article_sub)
                                        <input type="hidden" value="{{ $article_sub->languageid ?? '' }}"
                                            id="sel_lang{{ $article_sub->languageid }}" name="sel_lang[]">

                                        <div class="col-sm-6 mb-btm" id="div{{ $article_sub->id }}">
                                            <label class="my-1 mr-2" for="inlineFormCustomSelectPref">Article type in
                                                {{ $article_sub->name }} <span class="redalert"> *</span></label>
                                            <input id="title{{ $article_sub->id }}" type="text"
                                                class="form-control title_validation @error('title') is-invalid @enderror"
                                                name="title[]" value="{{ $article_sub->title ?? old('title.' . $i) }}"
                                                rel="{{ $article_sub->id }}" required autocomplete="title"
                                                placeholder="Enter {{ $article_sub->name }} here" autofocus>
                                            <span class="ErrP redalert titleerr1" style="display: none;">Please Check the
                                                {{ $article_sub->title }} title Entered</span>
