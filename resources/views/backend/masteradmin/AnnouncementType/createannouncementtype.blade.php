...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header text-white card-header-main">{{ isset($edit_f) ? 'Update' : 'Add' }} Alert type
                </div>
                ...
                <div class="card-body">
                    ...
                    @endif
                    @if (isset($edit_f))
                    <form id="formiid" method="POST" action="{{ route('masteradmin.updateannouncementtype') }}"
                        enctype="multipart/form-data">
                        @else
                        <form id="formiid" method="POST" action="{{ route('masteradmin.storeannouncementtype') }}"
                            enctype="multipart/form-data">
                            @endif
                            ...
                            <div class="row mb-3">
                                ...
                                @if (isset($keydata->id))
                                @foreach ($keydata->announcetypesub as $announcetypesub)
                                <input type="hidden" value="{{ $announcetypesub->languageid ?? '' }}"
                                    id="sel_lang{{ $announcetypesub->languageid }}" name="sel_lang[]">

                                <div class="col-sm-6 mb-btm" id="div{{ $announcetypesub->id }}">
                                    <label class="my-1 mr-2" for="inlineFormCustomSelectPref">Alert type in
                                        {{ $announcetypesub->name }} <span class="redalert"> *</span></label>
                                    <input id="title{{ $announcetypesub->id }}" type="text"
                                        class="form-control title_validation @error('title') is-invalid @enderror"
                                        name="title[]" value="{{ $announcetypesub->title ?? old('title.' . $i) }}"
                                        rel="{{ $announcetypesub->id }}" required autocomplete="title"
                                        placeholder="Enter {{ $announcetypesub->name }} here" autofocus>
                                    <span class="ErrP redalert titleerr1" style="display: none;">Please Check the
                                        {{ $announcetypesub->title }} title Entered</span>
                                ...
                                <div class="col-sm-6 mb-btm" id="div{{ $langs->id }}">
                                    ...
                                    <input id="title{{ $langs->id }}" type="text"
                                        class="form-control title_validation @error('title') is-invalid @enderror"
                                        rel="{{ $langs->id }}" name="title[]" value="" required
                                        autocomplete="title" placeholder="Enter {{ $langs->name }} here" autofocus>
                                    <span class="ErrP redalert titleerr1" style="display: none;">Please Check the
