...
<div class="container">
    <div class="row justify-content-center">
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header text-white card-header-main">{{isset($edit_f) ? 'Update' : 'Add'}} Linktype</div>
                ...
                <div class="card-body">
                    ...
                    @endif
                   @if(isset($edit_f))
                    <form id="formiid" method="POST" action="{{ route('updatelinktype') }}" enctype="multipart/form-data">
                    @else
                    <form id="formiid" method="POST" action="{{ route('storelinktype') }}" enctype="multipart/form-data">
                    @endif
                    ...
                    @csrf 
                        ...
                        <div class="row mb-3">
                            ...
                            @if(isset($edit_f)) 
                             ...
                             @if(isset($keydata->id)) @foreach(($keydata->linktype_sub) as $linksub)
                 
                             <input type="hidden"  value="{{$linksub->languageid ?? ''}}" id="sel_lang{{$linksub->languageid}}" name="sel_lang[]">

                            <div class="col-sm-6 mb-btm" id="div{{$linksub->id}}"> 
                                <label class="my-1 mr-2" for="inlineFormCustomSelectPref">Link type in {{$linksub->name}} <span class="redalert"> *</span></label>
                                <input id="title{{$linksub->id}}" type="text" class="form-control title_validation @error('title') is-invalid @enderror" name="title[]" value="{{ $linksub->title ?? old('title.'.$i)}}" rel="{{$linksub->id}}" required autocomplete="title" placeholder="Enter {{$linksub->name}} here" autofocus  >
                                <span class="ErrP redalert titleerr1" style="display: none;">Please Check the {{$linksub->title}} title Entered</span>
                                <span class="ErrP redalert titleerr2" style="display: none;">Please Check the {{$linksub->title}} title Entered</span>
                            </div>
                             @endforeach @endif
                        ...
                        <div class="row">
                            <div class="col-sm-10 offset-sm-2">
                               ...
                               @endif
                               <a type="submit" class="submitBtn btn btn-success" href="{{route('createlinktype')}}">Refresh</a>
                            </div>
                        </div>
                    </form>
