    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            <div class="card" id="entry_div">
                <div class="card-header text-white card-header-main">{{isset($edit_f) ? 'Update' : 'Add'}} Wellness</div>
                <div class="card-body">
                    @if(session('success'))
                    ...
                    @endif
                    @if(isset($edit_f))
                    <form id="formid" method="POST" action="{{ route('siteadmin.updatecreateWellnessTips') }}" enctype="multipart/form-data">
                        @else
                        <form id="formid" method="POST" action="{{ route('siteadmin.storeWellnessTips') }}" enctype="multipart/form-data">
                            @endif
                            ...
                            <div class="row mb-3 card-header card-main">
                                ...
                                <div class="row pt-3">
                                    <div class="col-sm-6">
                                        <label for="gallerytype" class="my-1 mr-2">Wellness type<span class="redalert"> *</span></label>
                                        <select class="form-control select2 formselect" name="wellnesstype" id="wellnesstype" required>
                                            <option value="">Select</option>

                                            @foreach($wellnessTipTypes as $wellnessTipType)
                                            @foreach ($wellnessTipType->wellnessTipTypesub as $wellnessTipTypesub)
                                            <option value="{{$wellnessTipType->id}}" @if(isset($edit_f)) {{ $wellnessTipType->id == $keydata->wellnessTipTypeId ? 'selected' : '' }} @endif>{{$wellnessTipTypesub->title}}</option>
                                            @endforeach
                                            @endforeach
                                        </select>
                                ...
                                @if(isset($keydata->id)) @foreach(($keydata->wellnessTipsub) as $wellnessTipsub)
                                <input type="hidden" value="{{$wellnessTipsub->languageid ?? ''}}" id="sel_lang" name="sel_lang[]">
                                <div class="row div_lan1 mb-3 pt-3">
                                    <div class="col-sm-12 mb-btm" id="div{{$wellnessTipsub->id}}">
                                        <label class="my-1 mr-2" for="inlineFormCustomSelectPref">Wellness Title in {{$wellnessTipsub->lang_sel->name}} <span class="redalert"> *</span></label>
                                        <input id="title{{$wellnessTipsub->id}}" type="text" class="form-control title title_validation @error('title') is-invalid @enderror" name="title[]" value="{{ $wellnessTipsub->title ?? old('title.'.$i)}}" rel="{{$wellnessTipsub->id}}" required autocomplete="title" placeholder="Enter main {{$wellnessTipsub->title}} here" autofocus>
                                        <span class="ErrP redalert titleerr1 display_status">Please Check the {{$wellnessTipsub->name}} title Entered</span>
                                        <span class="ErrP redalert titleerr2 display_status">Please Check the {{$wellnessTipsub->name}} title Entered</span>
                                    </div><br />
                                </div><br>
                                <div class="row div_lan1 mb-3">
                                    <div class="col-sm-12 mb-btm" id="div{{$wellnessTipsub->id}}">
                                        <label class="my-1 mr-2" for="inlineFormCustomSelectPref">Wellness Description in {{$wellnessTipsub->name}} <span class="redalert"> *</span></label>
                                        <textarea id="description{{$wellnessTipsub->id}}" type="text" class="form-control ckeditor @error('description') is-invalid @enderror" name="description[]" value="{{ $keydata->title ?? old('alt_title')}}" required autocomplete="description" placeholder="Enter description {{$wellnessTipsub->name}} here" autofocus >{{$wellnessTipsub->description}}</textarea>                                        
                                        <span class="ErrP redalert titleerr1 display_status">Please Check the {{$wellnessTipsub->name}} title Entered</span>
                                        <span class="ErrP redalert titleerr2 display_status">Please Check the {{$wellnessTipsub->name}} title Entered</span>
                                    </div><br />
                                </div><br>
                                ...
                                <div class="row div_lan1 mb-3">
                                    <div class="col-sm-12 mb-btm" id="div{{$langs->id}}">
                                        <label class="my-1 mr-2" for="inlineFormCustomSelectPref">Wellness Title in {{$langs->name}} <span class="redalert"> *</span></label>
                                ...
                                <div class="row div_lan1 mb-3">
                                    <div class="col-sm-12 mb-btm" id="div{{$langs->id}}">
                                        <label class="my-1 mr-2" for="inlineFormCustomSelectPref">Wellness Description in {{$langs->name}} <span class="redalert"> *</span></label>
                                        <textarea id="description{{$langs->id}}" type="text" class="form-control ckeditor @error('description') is-invalid @enderror" name="description[{{$i}}]" value="{{ $keydata->title ?? old('alt_title')}}" required autocomplete="description" placeholder="Enter description {{$langs->name}} here" autofocus ></textarea>                                        <span class="ErrP redalert titleerr1 display_status">Please Check the {{$langs->name}} title Entered</span>
                                        <span class="ErrP redalert titleerr2 display_status">Please Check the {{$langs->name}} title Entered</span>
                                    </div><br />
                                </div><br>
                                ...
                                <div class="row my-md-2 my-1 py-2 px-2 regform">
                                    ...
                                    <div class="col-md-6 d-flex justify-content-end" id="uploadPreview">
                                    </div> <!-- ./col6 -->
                                </div>
                                <!-- ./inner-card-iterate -->
                                <div><br /></div>
