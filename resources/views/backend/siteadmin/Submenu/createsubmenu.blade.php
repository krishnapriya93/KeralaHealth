...
<script>
 $( document ).ready(function() {
    ...
    $('.keyiderr').hide();
    $.ajax({
        url: "{{route('siteadmin.ordernumberchecksubmenu')}}",
        type : 'GET',
...
// alert(menulinktype_id_edit);
        ...
        //   alert(mainmenu_edit);
          $('#articletype').empty();
            ...
            // $('.degree').val(data.degree).attr('selected',true);
            $.each(response, function(index, element) {
                console.log(element.articleval_sub);
                $.each(element.articleval_sub, function(index1, element1) {
                    // console.log(element1);
                    if(article_id_edit==element1.articleid){
                        $('#articletype').append(
                    $('<option></option>').val(element.id).html(element1.title).attr('selected','selected')
                );
                    }else{

                        $('#articletype').append(
                    $('<option></option>').val(element.id).html(element1.title)
                );
                    }
