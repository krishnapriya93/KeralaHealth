...
class AdminController extends Controller
{
    ...
    public function user()
    {
        ...
        }])->with(['designationdata' => function($q1){
...
// }])->where('id', $role_id)->first();
      ...
      $breadcrumb = [
                ...
                1 => ['title' => 'User details', 'message' => 'User details', 'status' => 1],
            ];
            $data = $data->reject(fn($result) => in_array($result->id, [1,2,3,4,5,6,7]));
        }else{
            $breadcrumb = array(
                0 => array('title' => 'Home', 'message' => 'Home', 'status' => 0, 'link' => '/masteradminhome'),
        ...
        ->orderBy(function($query){
            $query->select('title') // Replace 'some_field' with the actual field you want to order by
                  ->from('officesub')
                  ->whereColumn('officesub.officeid', 'offices.id')
                  ->orderBy('title', 'asc') // Order by the same field
                  ->limit(1); // Limit to 1 to get the correct ordering
        })
        ->get();
        ...
        return view('backend.admin.Users.user', compact('data', 'usertype', 'navbar', 'user','designations','offices','role_type'));
    }
    ...
    public function edituser($id)
    {
        ...
        ->get();
        return view('backend.admin.Users.user', compact('data', 'edit_f', 'error', 'keydata', 'breadcrumbarr', 'navbar', 'user', 'usertype','designations','offices','role_type'));
    }
    ...
    public function updateuser(Request $request)
    {
        ...
        try {
            ...
            $request->input();
            $resDetail = User::where('id', $request->hidden_id)->first();
            ...
            if ($resDetail) {
                ...
                UserHistory::create([
                    ...
                    'role_id' => $resDetail->role_id,
                    'mobile' => $resDetail->mobile,
                    'designation' => $resDetail->designation,
                    'office_id' => $resDetail->office_id,
                    'created_at' => now(),
                ]);
            } else {
                return back()->withErrors('Not Updated');
    ...
    {
        $data = Component::where('status_id', 1)->get();
        $breadcrumb = array(
            0 => array('title' => 'Home', 'message' => 'Home', 'status' => 0, 'link' => '/masteradminhome'),
